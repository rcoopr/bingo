...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly create bingo-stack-31be
  fly create bingo-stack-31be-staging
  ```
...
- Add a `SESSION_SECRET`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE`,`SUPABASE_ANON_PUBLIC`, `SERVER_URL` and `DATABASE_URL` to your fly app secrets
  ...
  ```sh
  # staging
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app bingo-stack-31be-staging
  fly secrets set SUPABASE_URL="https://{YOUR_STAGING_INSTANCE_NAME}.supabase.co" --app bingo-stack-31be-staging
  fly secrets set SUPABASE_SERVICE_ROLE="{STAGING_SUPABASE_SERVICE_ROLE}" --app bingo-stack-31be-staging
  fly secrets set SUPABASE_ANON_PUBLIC="{STAGING_SUPABASE_ANON_PUBLIC}" --app bingo-stack-31be-staging
  fly secrets set DATABASE_URL="postgres://postgres:{STAGING_POSTGRES_PASSWORD}@db.{STAGING_YOUR_INSTANCE_NAME}.supabase.co:5432/postgres" --app bingo-stack-31be-staging
  fly secrets set SERVER_URL="https://{YOUR_STAGING_SERVEUR_URL}" --app bingo-stack-31be-staging

  # production
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app bingo-stack-31be
  fly secrets set SUPABASE_URL="https://{YOUR_INSTANCE_NAME}.supabase.co" --app bingo-stack-31be
  fly secrets set SUPABASE_SERVICE_ROLE="{SUPABASE_SERVICE_ROLE}" --app bingo-stack-31be
  fly secrets set SUPABASE_ANON_PUBLIC="{SUPABASE_ANON_PUBLIC}" --app bingo-stack-31be
  fly secrets set DATABASE_URL="postgres://postgres:{POSTGRES_PASSWORD}@db.{YOUR_INSTANCE_NAME}.supabase.co:5432/postgres" --app bingo-stack-31be
  fly secrets set SERVER_URL="https://{YOUR_STAGING_SERVEUR_URL}" --app bingo-stack-31be
  ```
